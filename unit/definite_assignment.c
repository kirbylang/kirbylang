...
// --- PendingSet API test ---
//
// pendingSetAdd/Contains/Remove/Clone are internal to the pass (not part
// of definite_assignment.h) -- only init/free are public, for callers
// like typecheckProgram()'s top-level pending set.

static void test_pending_set_starts_empty(void) {
  PendingSet set;
  pendingSetInit(&set);
  assert(set.count == 0);
  pendingSetFree(&set);
...
// --- checkDefiniteAssignment, called directly on a parsed FunctionNode ---
...
static void test_check_definite_assignment_direct_read_before_assign(void) {
  typecheckResetError();
  FunctionNode *fn = parseFirstFunction("fun f(): f64 { var x: f64; return x; }");
...
// --- whole-program driver tests (typecheckProgram already runs this pass) ---
